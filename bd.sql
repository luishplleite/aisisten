...
CREATE TABLE public.restaurants (
  ...
  payment_methods jsonb DEFAULT '{"cash": {"enabled": true, "needs_change": false}, "card": {"enabled": false, "debit": false, "credit": false}, "pix": {"enabled": false, "key": null}}'::jsonb,
  asaas_config jsonb DEFAULT '{}'::jsonb,
  whatsapp_config jsonb DEFAULT '{"enabled": false, "api_key": null, "phone_number": null}'::jsonb,
  smtp_config jsonb DEFAULT '{"enabled": false, "host": null, "port": null, "username": null, "password": null}'::jsonb,
  ...
  subscription_status character varying(50) DEFAULT 'trial' CHECK (subscription_status IN ('trial', 'active', 'expired', 'cancelled')),
...
-- ===== ATUALIZAÇÕES PARA CONFIGURAÇÕES AVANÇADAS =====
-- Executar apenas se as colunas não existirem

-- Atualizar notification_settings para incluir configurações de email e som
UPDATE public.restaurants 
SET notification_settings = '{"email": {"new_order": false, "daily_report": false}, "sound": {"new_order": true, "order_update": false}}'::jsonb
WHERE notification_settings = '{"email": true, "sound": true, "whatsapp": true}'::jsonb;

-- Adicionar colunas para métodos de pagamento se não existir
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'restaurants' AND column_name = 'payment_methods') THEN
        ALTER TABLE public.restaurants 
        ADD COLUMN payment_methods jsonb DEFAULT '{"cash": {"enabled": true, "needs_change": false}, "card": {"enabled": false, "debit": false, "credit": false}, "pix": {"enabled": false, "key": null}}'::jsonb;
    END IF;
END $$;

-- Adicionar colunas para configurações do WhatsApp se não existir
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'restaurants' AND column_name = 'whatsapp_config') THEN
        ALTER TABLE public.restaurants 
        ADD COLUMN whatsapp_config jsonb DEFAULT '{"enabled": false, "api_key": null, "phone_number": null}'::jsonb;
    END IF;
END $$;

-- Adicionar colunas para configurações SMTP se não existir
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'restaurants' AND column_name = 'smtp_config') THEN
        ALTER TABLE public.restaurants 
        ADD COLUMN smtp_config jsonb DEFAULT '{"enabled": false, "host": null, "port": null, "username": null, "password": null}'::jsonb;
    END IF;
END $$;

-- Tabela para formas de pagamento customizadas
CREATE TABLE public.custom_payment_methods (
  ...
  icon character varying DEFAULT 'fa-money-bill',
  display_order integer DEFAULT 0,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT custom_payment_methods_pkey PRIMARY KEY (id),
  CONSTRAINT custom_payment_methods_restaurant_id_fkey FOREIGN KEY (restaurant_id) REFERENCES public.restaurants(id) ON DELETE CASCADE
);

-- Adicionar coluna operating_hours para compatibilidade
DO $$ 
BEGIN
...
COMMENT ON COLUMN public.restaurants.notification_settings IS 'Configurações de notificações estruturadas: email (new_order, daily_report), sound (new_order, order_update)';
COMMENT ON COLUMN public.restaurants.payment_methods IS 'Métodos de pagamento aceitos: cash, card (debit/credit), pix com chave';
COMMENT ON COLUMN public.restaurants.whatsapp_config IS 'Configurações de integração com WhatsApp';
COMMENT ON COLUMN public.restaurants.smtp_config IS 'Configurações de servidor SMTP para envio de emails';
COMMENT ON COLUMN public.restaurants.business_hours IS 'Horários de funcionamento por dia da semana em formato JSON (formato antigo)';
COMMENT ON COLUMN public.restaurants.operating_hours IS 'Horários de funcionamento por dia da semana em formato JSON (formato novo)';
COMMENT ON TABLE public.custom_payment_methods IS 'Formas de pagamento customizadas por restaurante';
